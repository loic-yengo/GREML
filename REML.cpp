...
REML::REML(string grmInput, string phenoFile, string covarFile,int mpheno, int nGRMs,bool verbose, ofstream &fileLog){
  ...
  for(int k=1;k<nGRMs;k++){
    ...
    MatrixXf iG = MatrixXf::Zero(Ngrms(k),Ngrms(k));
...
void REML::summaryGRMs(bool verbose, ofstream &fileLog){
  if(this->nGRMs==1){
...
void REML::ai_reml(VectorXf &vars0,int maxit, float tol, int nrand,int seed, bool verbose, ofstream &fileLog){
  float time_elapsed;
  float traceV_  = 0.;
  float traceP   = 0.;
  ...
  VectorXf xTV_y   = VectorXf::Zero(n);
  VectorXf xTV_Py  = VectorXf::Zero(n);
  ...
  while(it<maxit and eps>tol){
    ...
    if(success_llt==0){
      ...
      V_X   = llt_V.solve(X);
      V_y   = llt_V.solve(y);
      xTV_x = X.transpose() * V_X;
      
      llt_xTV_x.compute(xTV_x);
      
      xTV_y  = V_X.transpose() * y;
      Py     = V_y - V_X * llt_xTV_x.solve(xTV_y);
      ...
      // Calculate PPy
      V_Py   = llt_V.solve(Py);
      xTV_Py = V_X.transpose() * Py;
      PPy    = V_Py - V_X * llt_xTV_x.solve(xTV_Py);
      
      // Calculate PGPy
      for(int k=0;k<nGRMs;k++){
        V_GPy[k]   = llt_V.solve(GPy[k]);
        xTV_GPy[k] = V_X.transpose() * GPy[k];
        PGPy[k]    = V_GPy[k] - V_X * llt_xTV_x.solve(xTV_GPy[k]);
      }
      
      // Calculate tools for Traces
      xTV_2x       = V_X.transpose() * V_X;   // X'V^-2X
      xTV_x_xTV_2x = llt_xTV_x.solve(xTV_2x); // (x'V^-1x)^-1 (X'V^-2X)
      
      for(int k=0;k<nGRMs;k++){
        GV_X[k] = G[k] * V_X;
        xTV_GV_x[k] = V_X.transpose() * GV_X[k];
        xTV_x_xTV_GV_x[k] = llt_xTV_x.solve(xTV_GV_x[k]);
      }
      
      // Simulate Xrand matrix -- n x nrand
      L_x  = llt_V.matrixL().transpose().solve(x);
      for(int k=0;k<nGRMs;k++){
        GL_x[k] = G[k].transpose() * L_x; //G * L_x;
      }
      ...
      if(verbose){
        ...
        cout<<" - V(e) = "<<vars(nGRMs)<<" - eps = "<<eps<<" ["<<time_elapsed<<" sec.].\n";
      }
      fileLog<<"# [AI-REML] it = "<<it;
      for(int k=0;k<nGRMs;k++){
        fileLog<<" - Vg("<<1+k<<") = "<<vars(k);
      }
      fileLog<<" - V(e) = "<<vars(nGRMs)<<" - eps = "<<eps<<" ["<<time_elapsed<<" sec.].\n";
    }else{
...
void REML::writeResults(string outREML, bool verbose, ofstream &fileLog){
