...
int main(int argc, char *argv[]){
  ...
  if (sw == "--help" or sw == "-h"){
    ...
    cerr<<"\t--pheno      : Phenotype file just a single column - no missing values."<<endl;
    ...
    cerr<<"\t--on-disk    : Run analyses on disk (does not load ang GRM in memory)."<<endl;
  ...
  if(grmPrefix=="" and mgrmFiles==""){
    ...
    if(grmPrefix!="" and mgrmFiles!=""){
      ...
      if(vars0.sum() <= 0.0){
        cerr<<"\tSum of initial variance components should be positive. [Use --init][Check help: --help]."<<endl;
        exit(1); 
      }
      //cout<<vars<<endl;
    }
  ...
  fileLog << (now->tm_year + 1900) << '-'
          ...
          <<  ".\n";
  if(onDisk){
    fileLog<<"# Analyses running on disk.\n";
  }
  ...
  if(onDisk){
    REML_d myGREML = REML_d(grmInput,phenoFile,covarFile,
    ...
    REML_f myGREML = REML_f(grmInput,phenoFile,covarFile,
                        mpheno,nGRMs,verbose,fileLog);
